...
| post_handshake_auth \[\[this document]]  |      CH, EE |
...
A client MAY send the "post_handshake_auth" extension in the ClientHello
message. The server MUST include the "post_handshake_auth" extension in the
...
When the "post_handshake_auth" extension is negotiated (see
...
A client that receives a CertificateRequest message without having negotiated
the "post_handshake_auth" extension MUST send a "unexpected_message" fatal
alert.
