...
: The latest (highest valued) version of the TLS protocol offered by the
  client. This SHOULD be the same as the latest version supported. It MAY be
  a higher version if the negotiation is attempted by a client of a newer (as
  of now unspecified) protocol. For this version of the specification, the
  version will be { 3, 4 }. (See {{backward-compatibility}} for details about
...
set of parameters, the server MUST respond with a "handshake_failure" fatal
...
: The length (in bytes) of the following TLSPlaintext.fragment. The
  length MUST NOT exceed 2^14. Peer receiving TLSPlaintext larger than
  that MUST send a fatal "record_overflow" alert and close the connection.
