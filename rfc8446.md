...
* Authentication: The server side of the channel is always
  ...
  (e.g., RSA {{RSA}}, the Elliptic Curve Digital Signature
...
- Other cryptographic improvements were made, including the removal of compression and
  custom DHE groups, changing the RSA padding to use 
...
~~~
{: #tls-full title="Message Flow for a Full TLS Handshake"}
...
: Indicates a signature algorithm using ECDSA {{ECDSA}}, the corresponding
  curve as defined in ANSI X9.62 {{ECDSA}} and FIPS 186-4 {{DSS}}, and the
...
: Indicates support for the corresponding named curve, defined
  in eithr FIPS 186-4 {{DSS}} or in {{RFC7748}}.
...
(e.g., ones not in the PSK database or are encrypted with an
...
: A set of extension values for the CertificateEntry. The "Extension"
  ...
  at present include the OCSP Status extension {{RFC6066}} and
...
not send an "early_data" extension extension in EncryptedExtensions, then the client MUST NOT send an
...
Tthe encrypted_record field of TLSCiphertext is set to AEADEncrypted.
...
- "0" indicates a string of Hash-lengths bytes set to zero.
...
For X25519 and X448, the ECDH calculations are used as follows:
...
the second form of attack described above in {{anti-replay}}.
...
cipher suites for TLS 1.2 cannot be used with TLS 1.3.
...
- Do you correctly handle handshake messages that are fragmented to
  ...
  corner cases like a ClientHello that is split to several small fragments? Do
...
: The shared session keys should be known only to the communicating
  parties and not to the attacker (see {{CK01}}; Definition 1, part 2).
...
: The cryptographic parameters should be the same on both sides and
  ...
  absence of an attack (see {{BBFKZG16}}; Definitions 8 and 9).
...
TLS 1.3 are derived from the same Handshake cCntexts as the
...
See {{BBFKZG16}} for more details on TLS 1.3 and downgrade.
...
construction of the serial generator as discussed in [REKEY], which shows that rekeying can
...
{{BMMT15}}, {{BT16}}, {{BDFKPPRSZZ16}}, {{BBK17}}, and {{Anon18}}.
