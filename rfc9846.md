...
informative:
  ...
  HGFS15:
       ...
       -
         ins: C. Schanes
       refcontent: 9th USENIX Workshop on Offensive Technologies (WOOT 15)
       date: 2015
  FGSW16:
       ...
       -
         ins: B. Warinschi
       refcontent: 2016 IEEE Symposium on Security and Privacy (SP), pp. 452-469
       seriesinfo:
  ...
  BDFKPPRSZZ16:
      ...
      -
        ins: J. Zinzindohoue
      refcontent: Cryptology ePrint Archive, Paper 2016/1178
      date: 2016
  ...
  BMMRT15:
      ...
      -
        ins: B. Tackmann
      refcontent: Cryptology ePrint Archive, Paper 2015/394
      date: 2015
  ...
  BT16:
      ...
      -
        ins: B. Tackmann
      refcontent: Cryptology ePrint Archive, Paper 2016/564
      date: 2016
  ...
  Kraw16:
       ...
       date: 2016
       refcontent: Cryptology ePrint Archive, Paper 2016/711
  ...
  KW16:
       ...
       date: 2015
       refcontent: Cryptology ePrint Archive, Paper 2015/978
  ...
  DFGS15:
       ...
       date: 2015
       refcontent: Cryptology ePrint Archive, Paper 2015/914
  ...
  DFGS16:
       title: "A Cryptographic Analysis of the TLS 1.3 draft-10 Full and Pre-shared Key Handshake Protocol"
       date: 2016
       refcontent: Cryptology ePrint Archive, Paper 2016/081
  ...
  FG17:
       ...
       date: 2017
       refcontent: Cryptology ePrint Archive, Paper 2017/082
  ...
  Kraw10:
       ...
       date: 2010
       refcontent: Cryptology ePrint Archive, Paper 2010/264
  ...
  Selfie:
       ...
       date: 2019
       refcontent: Cryptology ePrint Archive, Paper 2019/347
...
layer on top of TLS transparently. However, the TLS standard does not
...
<!--[rfced] The SVG in Figures 1 and 4 are outputting a solid circle for this text, while the figure displays *.  Please review.  One possible fix would be to move the legend outside of the figure.  Please review and let us know how this may be updated.
...
Finished:
: A Message Authentication Code (MAC) over the entire handshake.
...
- A list of cipher suites that indicates the AEAD algorithm / HKDF hash
  pairs which the client supports.
- A "supported_groups" ({{supported-groups}}) extension which indicates the (EC)DHE groups
  that the client supports and a "key_share" ({{key-share}}) extension which contains
...
- A "signature_algorithms" ({{signature-algorithms}}) extension which indicates the signature
  algorithms that the client can accept. A "signature_algorithms_cert" extension ({{signature-algorithms}}) may also be
...
: A list of extensions.  The ServerHello MUST only include extensions
  ...
  the "pre_shared_key" extension, "key_share" extension, or both (when using
...
Note: This table only includes extensions marked as
...
Finite Field DHE (FFDHE) groups.  The key_exchange values for each KeyShareEntry MUST be
...
(for instance, see {{ticket-establishment}}), the client can send Application Data
...
ticket. However, clients SHOULD store the SNI with the PSK to fulfill
...
SHOULD select a single PSK and solely validate the binder that
...
request a certificate from the client. If sent, this message MUST
...
but not including record layer headers. That is,
...
type "message_hash" containing Hash(ClientHello1). That is,
...
running transcript hash value based on the negotiated hash. However,
note that subsequent post-handshake authentications do not include
...
<!--[rfced] We believe the intention of this line to note that the asterisk
has a specific meaning when present.  Please note that we will update the XML to treat this as <dl>.  Currently, kramdown treats this as a bulleted list item, and definition list yeilds the following: 
...
Because the size of sequence numbers is 64 bits, they should not
...
layer.  However, padding for encrypted Handshake or Alert records must
still be handled at the TLS layer.  Later documents may define
...
: Sent by servers when a client
  "application_layer_protocol_negotiation" extension only advertises
...
the round-trip time between client and server. That is,
...
Security issues are discussed throughout this memo, especially in Appendices
{{implementation-notes}}{: format="counter"}, {{backward-compatibility}}{: format="counter"}, and {{security-analysis}}{: format="counter"}.
...
- "TLS HandshakeType" registry: Future values are allocated via
  ...
  Values marked as "_RESERVED" have comments describing their previous or
  temporary usage.
...
- Do you zero-pad Diffie-Hellman public key values and shared
  secrets to the group size (see Sections {{ffdhe-param}}{: format="counter"} and {{finite-field-diffie-hellman}}{: format="counter"})?
...
Encrypted Client Hello {{PRE-RFC9849}} extension can
...
the post-handshake phase possesses the traffic key. Therefore, this party is
...
long-term secret (signature key or external PSK) is compromised. Therefore, it
does not provide post-compromise security {{?CCG16=DOI.10.1109/CSF.2016.19}}, sometimes also referred to
as backward or future secrecy. This is in contrast to KCI resistance, which
...
Integrity:
: An attacker should not be able to craft a new record that is
...
If implemented correctly, the mechanisms described in Sections
{{single-use-tickets}}{: format="counter"} and {{client-hello-recording}}{: format="counter"} prevent a
...
PSK importers as defined in {{RFC9258}}, that prevents such malicious
