...
: 32 bytes generated by a secure random number generator.
  ...
  below if negotiating TLS 1.2, but the
...
TLS 1.3 clients receiving a ServerHello indicating TLS 1.2
MUST check that the last 8 bytes are not equal to this value.
...
that negotiate the use of TLS 1.2 SHOULD set the record layer
...
and TLS 1.2. For example, if the server supports TLS 1.2,
and legacy_version is TLS 1.2, the server will proceed with a TLS 1.2 ServerHello.
